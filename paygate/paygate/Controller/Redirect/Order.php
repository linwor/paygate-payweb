...
class Order extends \Paygate\Paygate\Controller\AbstractPaygate
{
    ...
    public function getStandardCheckoutFormFields( $order )
    {
        ...
        //Set REFERENCE in SESSION to use in redirect callback
        $reference = $order->getRealOrderId();
        $this->_checkoutSession->setMagReference( $reference );
        $this->_logger->debug( 'In Order: Reference: ' . $reference );
        $this->_logger->debug( 'In Order: Reference from session: ' . $this->_checkoutSession->getMagReference() );
        ...
        if ( $model->getConfigData( 'enable_ipn' ) != '0' ) {
            $fields['NOTIFY_URL'] = $this->_urlBuilder->getUrl( 'paygate/notify/index', array( '_secure' => true ) );
        }
        $fields['USER3'] = 'magento2-v2.2.0';
        ...
        $processData = array(
            ...
            'IFRAME'         => $model->getConfigData( 'iframe_mode' ),
        );
